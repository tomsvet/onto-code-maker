...
import ontology.generator.classes.examples.family.entities.Person;
...
public class PersonSerialization extends SerializationModel<Person>{
    ...
    protected void addPropertiesToModel(Model model,  Person person) {
        ...
        hasLuckyNumbersPom.addAll(person.getHasLuckyNumbers());
        setLiteralsRDFCollection(model,person.getIri(),Vocabulary.HASLUCKYNUMBERS_PROPERTY_IRI,hasLuckyNumbersPom);
        ...
        agePom.addAll(person.getAge());
        setLiteralsRDFCollection(model,person.getIri(),Vocabulary.AGE_PROPERTY_IRI,agePom);
    ...
    protected void setProperties(Model model,Person person,int nestingLevel) throws Exception{
        Set<Value> hasLuckyNumbers = super.getAllObjects(model,Vocabulary.HASLUCKYNUMBERS_PROPERTY_IRI,person.getIri());
        for(Value propValue:hasLuckyNumbers){
            if(propValue.isLiteral()) {
                Literal literalValue = (Literal) propValue;
                person.addHasLuckyNumbers(Integer.valueOf(literalValue.intValue()) );
        ...
        Set<Value> age = super.getAllObjects(model,Vocabulary.AGE_PROPERTY_IRI,person.getIri());
        for(Value propValue:age){
            if(propValue.isLiteral()) {
                Literal literalValue = (Literal) propValue;
                person.addAge(Integer.valueOf(literalValue.intValue()) );
            }else if(propValue.isBNode()){
                List<Value> listOfValues = super.getRDFCollection(model,(BNode)propValue);
                for(Value value:listOfValues){
                    Literal literalValue = (Literal) value;
